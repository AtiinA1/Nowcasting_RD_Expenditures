...
def init_wandb(config=None):
    """
    Override wandb init to tag this variant as AllVar.
    Keeps the same default config behavior as the base script.
    """
    # Ensure dataset-specific behavior
    ...
    cfg_override['dataset_variant'] = 'AllVar'
    # Delegate to the original base init so defaults are guaranteed
    cfg = _orig_init_wandb(cfg_override)
    # Append a variant-specific tag
    run = wandb.run
    current_tags = list(run.tags) if getattr(run, "tags", None) else []
    run.tags = current_tags + ["AllVar"]
    return cfg


# Monkey-patch the base init with our tagged variant
base.init_wandb = init_wandb

# Override create_train_test_splits to select AllVar columns (GT YTD + GT lags + RD lags + Macro lags)
def create_train_test_splits(merged_df, df_trends, rd_expenditure_df_rev, max_lag):
    ...
    # Build AllVar relevant columns from original AllVar code
    base_keys = ['Year', 'Month', 'Country']
    gt_ytd = [f"{col}_mean_YTD" for col in df_trends.columns if col not in ['Country', 'Year', 'Month']]
    gt_lags = []
    rd_lags = []
    macro_lags = []
    macro_vars = ['gdpca', 'unemp_rate', 'population', 'inflation', 'export_vol', 'import_vol']
    for lag in range(1, max_lag+1):
        gt_lags += [f"{col}_yearly_avg_lag{lag}" for col in df_trends.columns if col not in ['Country', 'Year', 'Month']]
        rd_lags += [f"{col}_lag{lag}" for col in rd_expenditure_df_rev.columns if col not in ['Country', 'Year', 'Month']]
        macro_lags += [f"{var}_lag{lag}" for var in macro_vars]

    # Keep only columns that exist
    desired = [c for c in base_keys + gt_ytd + gt_lags + rd_lags + macro_lags if c in X_all.columns]
