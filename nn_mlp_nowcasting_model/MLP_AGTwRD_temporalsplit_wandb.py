...
def init_wandb(config=None):
    """
    Override wandb init to tag this variant as AGTwRD.
...
# Override create_train_test_splits to select AGTwRD columns (GT yearly_avg_lag + RD lags)
def create_train_test_splits(merged_df, df_trends, rd_expenditure_df_rev, max_lag):
    ...
    # Build AGTwRD relevant columns from original AGTwRD code
    ...
    desired = [c for c in base_keys + gt_lags + rd_lags if c in X_all.columns]
