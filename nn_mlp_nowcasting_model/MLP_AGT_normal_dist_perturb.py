...
df_trends= pd.read_csv('/Users/atin/Nowcasting/data/GT/related_toic_approach/trends_data_by_topic_resampled_filtered.csv')
df_trends_monthly = pd.read_csv('/Users/atin/Nowcasting/data/GT/related_toic_approach/trends_data_by_topic_filtered.csv')
...
df_trends_monthly_aggregated.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/df_trends_monthly_YTD.csv')
...
df_trends_monthly_aggregated.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/df_trends_monthly_YTD_YR.csv')
print(df_trends_monthly_aggregated.columns)
...
merged_df.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/merged_df.csv')
...
merged_df.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/merged_df_test_RV01_batches_all_AggMGT_Evolution_0.csv")
...
X_all.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/X_all_AggMGT_Evolution.csv")
...
X_train.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/X_train_OHC_Month_Cat_Country.csv")
...
X_train.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/X_train_std.csv")
...
class Ensemble:
    ...
    def train(self, X_train, y_train, X_val, y_val, country_indices_train, country_indices_val, criterion, optimizer_class, num_epochs=num_epochs, batch_size=batch_size, patience=patience):
        ...
        for idx, model in enumerate(self.models):
            ...
            best_model_path = f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/best_model_{idx}.pt"
...
# Save to CSV
csv_path = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/ensemble_info.csv"
...
adjusted_r_squared_csv_path = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/adjusted_r_squared_values.csv"
...
csv_path = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/df_combined_pred_vs_true.csv"
...
def plot_scatter_per_country(df, title_prefix):
    ...
    for country in countries:
        ...
        plt.grid(True)
        plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/{title_prefix}_Predictions_vs_TrueValues_{country}.pdf")        
    ...
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/{title_prefix}_Predictions_vs_TrueValues_Combined.pdf")
    plt.close()  # Close the individual plot
...
def plot_scatter_per_country_log(df, title_prefix):
    ...
    for country in countries:
        ...
        plt.grid(True)
        plt.yscale('log')
        plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/{title_prefix}_Predictions_vs_TrueValues_{country}_log.pdf")        
        plt.close()  # Close the individual plot
    ...
    plt.yscale('log')
    plt.legend(title="Country")
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/{title_prefix}_Predictions_vs_TrueValues_Combined_log.pdf")
    plt.close()  # Close the individual plot
...
# Save back to CSV
metrics_df.to_csv("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/metrics_df_minibatchGD.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.train_losses):
    ...
    # Save to CSV
    df_train_loss.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/MLP_MinibatchGD_Training_Loss_Iterations_Model_{idx+1}.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.val_losses):
    ...
    # Save to CSV
    df_val_loss.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/MLP_MinibatchGD_Validation_Loss_Iterations_Model_{idx+1}.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.train_losses):
    ...
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/MLP_MinibatchGD_Training_Loss_Iterations_Model_{idx+1}.pdf")
    #plt.show()
...
for idx, model_losses in enumerate(ensemble.val_losses):
    ...
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/MLP_MinibatchGD_Validation_Loss_Iterations_Model_{idx+1}.pdf")
    #plt.show()
...
# Save to CSV
df.to_csv("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/country_metrics.csv", index=False)
...
for model in df['Models'][0]:
    ...
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/country_metrics_vis_{model}.pdf")
    #plt.show()
...
def compute_elasticity_torch(model, X_sample, country_indices_sample, feature_index, num_perturbations, perturb_distribution):
    model.eval()  # Switch to evaluation mode
    ...
    for _ in range(num_perturbations):
        if perturb_distribution is None:
            perturb_amount = 0.01  # Default to 1% increase if no distribution is provided
        else:
            perturb_amount = perturb_distribution()
    ...
    return elasticities  # return all elasticity values
...
# Ensure 'country_encoded' is of integer type
...
# Fit the LabelEncoder and decode the country_encoded values to actual country names
decoded_country_names = le.inverse_transform(X_train['country_encoded'].values)

# Define a perturbation distribution function (e.g., normal distribution with mean 0.01 and std dev 0.005)
...
for model in ensemble.models:
    ...
    for i in range(X_train_tensor.size(0)):
        ...
        for j, feature in enumerate(feature_names):
            if feature != 'Year':
                elasticities = compute_elasticity_torch(model, X_train_tensor[i:i+1], country_indices_train[i:i+1], j, num_perturbations=10, perturb_distribution=perturb_distribution)
                sample_elasticities.append(elasticities)
        
        # Fetch the decoded country name
        country_name = decoded_country_names[i]

        for feature, elasticities in zip(feature_names, sample_elasticities):
            mean_elasticity = np.mean(elasticities)
            lower_bound = np.percentile(elasticities, 2.5)
            upper_bound = np.percentile(elasticities, 97.5)
            ...
            model_elasticities.append({
                ...
                'Elasticity_Lower_Bound': lower_bound,
                'Elasticity_Upper_Bound': upper_bound
            })
...
df_elasticities_avg_wyr = df_elasticities_raw.groupby(['Country', 'Year', 'Feature']).agg({
    'Elasticity': 'mean',
    'Elasticity_Lower_Bound': 'mean',
    'Elasticity_Upper_Bound': 'mean'
}).reset_index()

df_elasticities_avg_woyr = df_elasticities_raw.groupby(['Country', 'Feature']).agg({
    'Elasticity': 'mean',
    'Elasticity_Lower_Bound': 'mean',
    'Elasticity_Upper_Bound': 'mean'
}).reset_index()
...
csv_path_elasticities_avg_wyr = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/elasticities_avg_ensemble_wyr.csv"
df_elasticities_avg_wyr.to_csv(csv_path_elasticities_avg_wyr, index=False)

csv_path_elasticities_avg_woyr = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/elasticities_avg_ensemble_woyr.csv"
df_elasticities_avg_woyr.to_csv(csv_path_elasticities_avg_woyr, index=False)

csv_path_elasticities_raw = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/elasticities_all_models.csv"
...
# Convert the results into a DataFrame
df_elasticities_raw_degenerate = pd.DataFrame(all_elasticities)

# Compute average elasticity for each feature per country (across all models)
df_elasticities_avg_wyr_degenerate = df_elasticities_raw_degenerate.groupby(['Country', 'Year', 'Feature']).agg({'Elasticity': 'mean'}).reset_index()

df_elasticities_avg_woyr_degenerate = df_elasticities_raw_degenerate.groupby(['Country', 'Feature']).agg({'Elasticity': 'mean'}).reset_index()

# Save to CSV if needed
csv_path_elasticities_avg_wyr_degenerate = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/elasticities_avg_ensemble_wyr_degenerate.csv"
df_elasticities_avg_wyr_degenerate.to_csv(csv_path_elasticities_avg_wyr_degenerate, index=False)

# Save to CSV if needed
csv_path_elasticities_avg_woyr_degenerate = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/elasticities_avg_ensemble_woyr_degenerate.csv"
df_elasticities_avg_woyr_degenerate.to_csv(csv_path_elasticities_avg_woyr_degenerate, index=False)

csv_path_elasticities_raw = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/elasticities_all_models_degenerate.csv"
df_elasticities_raw_degenerate.to_csv(csv_path_elasticities_raw_degenerate, index=False)
...
plt.savefig("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_summary_train.pdf", bbox_inches='tight')

plt.figure()
shap.summary_plot(shap_values_train_adj, X_train_df, plot_type="bar", show=False)
plt.savefig("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_bar_train_bar.pdf", bbox_inches='tight')

plt.figure()
shap.summary_plot(shap_values_val_adj, X_val_df, show=False)
plt.savefig("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_summary_val.pdf", bbox_inches='tight')

plt.figure()
shap.summary_plot(shap_values_val_adj, X_val_df, plot_type="bar", show=False)
plt.savefig("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_bar_val_bar.pdf", bbox_inches='tight')

plt.figure()
shap.summary_plot(shap_values_test_adj, X_test_df, show=False)
plt.savefig("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_summary_test.pdf", bbox_inches='tight')

plt.figure()
shap.summary_plot(shap_values_test_adj, X_test_df, plot_type="bar", show=False)
plt.savefig("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_bar_test_bar.pdf", bbox_inches='tight')

df_shap_train = pd.DataFrame(shap_values_train_adj, columns=feature_names)
df_shap_train.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_values_train.csv', index=False)

df_shap_val = pd.DataFrame(shap_values_val_adj, columns=feature_names)
df_shap_val.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_values_val.csv', index=False)

df_shap_test = pd.DataFrame(shap_values_test_adj, columns=feature_names)
df_shap_test.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/shap_values_test.csv', index=False)
