...
def init_wandb(config=None):
    """
    Override wandb init to tag this variant as LagRD.
    ...
    cfg_override['dataset_variant'] = 'LagRD'
...
# Override create_train_test_splits to select LagRD columns (RD lags only)
def create_train_test_splits(merged_df, df_trends, rd_expenditure_df_rev, max_lag):
    ...
    # Build LagRD relevant columns from original LagRD code
    ...
    desired = [c for c in base_keys + rd_lags if c in X_all.columns]
