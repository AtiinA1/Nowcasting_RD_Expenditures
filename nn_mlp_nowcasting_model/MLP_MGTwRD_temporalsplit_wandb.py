...
def init_wandb(config=None):
    """
    Variant wrapper for the MGTwRD temporalsplit run.
    Delegates to base init to ensure defaults, then appends an 'MGTwRD' tag.
    """
...
def create_train_test_splits(merged_df, df_trends, rd_expenditure_df_rev, max_lag):
    ...
    # Build MGTwRD relevant columns from original MGTwRD code
    ...
    desired = [c for c in base_keys + gt_ytd + gt_lags + rd_lags if c in X_all.columns]
