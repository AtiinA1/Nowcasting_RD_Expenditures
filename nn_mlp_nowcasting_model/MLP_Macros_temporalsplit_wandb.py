...
def init_wandb(config=None):
    """
    Variant wrapper for the Macros temporalsplit run.
    Delegates to base init to ensure defaults, then appends a 'Macros' tag.
    """
...
def create_train_test_splits(merged_df, df_trends, rd_expenditure_df_rev, max_lag):
    ...
    # Build Macros relevant columns from original Macros code
    ...
    desired = [c for c in base_keys + rd_lags + macro_lags if c in X_all.columns]
