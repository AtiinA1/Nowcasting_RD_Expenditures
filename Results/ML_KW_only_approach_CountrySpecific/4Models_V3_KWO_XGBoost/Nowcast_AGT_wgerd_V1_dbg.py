...
# Merge the two DataFrames
merged_df = pd.merge(oecd_df_rev, df_trends, on=['Country', 'Year'], how='left')
...
merged_df.to_csv('/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/merged_df.csv')
...
# Create the lagged values for all variables first
for lag in range(1, max_lag+1):
    for col in df_trends.columns:
        if col not in ['Country', 'Year']:
            # Create a new lagged column
            lagged_col_name = f'{col}_lag{lag}'
            merged_df[lagged_col_name] = merged_df.groupby('Country')[col].shift(lag)
...
cols_fillna_zero = [f"{col}_lag{lag}" for col in df_trends.columns if col not in ['Country', 'Year']]
...
for country in X_all['Country'].unique():
    ...
    for lag in range(1, max_lag+1):
        ...
        for cur_lag in range(1, lag+1):
            ...
            relevant_columns.update([f"{col}_lag{cur_lag}" for col in df_trends.columns if col not in ['Country', 'Year']] )
        ...
        #model_coeff.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/model_coeff_{country}_{lag}.csv")
        ...
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/model_scatter_train_{model_name}_{country}_{lag}.png")
        #plt.show()

        sns.set_theme(color_codes=True)
        fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
        plt.hist(y_train_country['residuals'])
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/model_residuals_train_{model_name}_{country}_{lag}.png")
        #plt.show()
        ...
        # new row content 
        row_dict = {'model': [model_name], 'tr_cv_scores_means': [cv_scores.mean()], 'tr_cv_scores_std': [cv_scores.std()],'ts_cv_score': [test_score]}
        ...
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/model_scatter_test_{model_name}_{country}_{lag}.png")
        #plt.show()
    
        sns.set_theme(color_codes=True)
        fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
        plt.hist(y_test_country['residuals'])
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/model_residuals_test_{model_name}_{country}_{lag}.png")
        #plt.show()
...
merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/merged_df_test_RV01_batches_all_{model_name}.csv")
...
evaluation_train_df.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/evaluation_batches_train_{model_name}.csv", index=False)
evaluation_test_df.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3_KWO_XGBoost/AGT/AGTwRD/evaluation_testAGT1_{model_name}.csv", index=False)
