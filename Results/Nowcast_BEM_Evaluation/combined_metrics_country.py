...
for variation in model_variations:
    test2_dir = os.path.join(base_dir, variation, "test10")
...
combined_df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model_Evaluations/test10/combined_country_results.csv", index=False)
...
def plot_metrics(metric):
    ...
    # Save the figure
    plt.savefig(f'/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model_Evaluations/test10/boxplot_{metric}.png', bbox_inches='tight')
    plt.show()
...
def plot_metrics_nofliers(metric):
    plt.figure(figsize=(12, 6))
    sns.boxplot(data=combined_df_rev, x='Algorithm', y=metric, hue='Model_Variation', showfliers=False)
    plt.title(f'Boxplot of {metric} by Model Variation and Algorithm')
    plt.ylabel(metric)
    plt.xlabel('Algorithm')
    plt.xticks(rotation=45)
    ...
    # Save the figure
    plt.savefig(f'/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model_Evaluations/test10/boxplot_nofliers_{metric}.png', bbox_inches='tight')
    plt.show()

# Plot for each metric
for metric in ['MAPE', 'RMSE', 'MAE']:
    plot_metrics_nofliers(metric)
...
def plot_zoomed_metrics(metric):
    ...
    plt.savefig(f'/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model_Evaluations/test10/zoomed_boxplot_{metric}.png', bbox_inches='tight')
    plt.show()
...
def plot_zoomed_metrics_nofliers(metric):
    plt.figure(figsize=(12, 6))
    sns.boxplot(data=filtered_df, x='Algorithm', y=metric, hue='Model_Variation', showfliers=False)  # Set showfliers to False here
    plt.title(f'Boxplot of {metric} by Model Variation (Zoomed)')
    plt.ylabel(metric)
    plt.xlabel('Algorithm')
    plt.xticks(rotation=45)
    ...
    # Save the zoomed figure
    plt.savefig(f'/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model_Evaluations/test10/zoomed_boxplot_noshowfliers_{metric}.png', bbox_inches='tight')
    plt.show()

# Plot for each metric
for metric in ['MAPE', 'RMSE', 'MAE']:
    plot_zoomed_metrics_nofliers(metric)
