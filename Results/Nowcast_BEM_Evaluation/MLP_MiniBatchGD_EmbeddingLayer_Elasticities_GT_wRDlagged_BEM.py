...
df_elasticities_YTD = df_elasticities[df_elasticities['Feature'].str.contains('_mean_YTD')]

# Prepare the df_elasticities_YTD DataFrame by removing the '_mean_YTD' suffix
df_elasticities_YTD['base_feature'] = df_elasticities_YTD['Feature'].str.replace('_mean_YTD', '')
...
df_elasticities_lagged = df_elasticities[df_elasticities['Feature'].str.contains('rd_expenditure_lag')]
...
for lag in lags:
    ...
    merged_df = pd.merge(df_trends_monthly, df_elasticities_lagged[df_elasticities_lagged['Feature'] == f"rd_expenditure_lag{lag}"], on='Country', how='left')
