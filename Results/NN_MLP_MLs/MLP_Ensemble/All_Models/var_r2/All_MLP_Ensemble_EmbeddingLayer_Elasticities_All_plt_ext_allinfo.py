...
df_trends_monthly_aggregated.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/df_trends_monthly_YTD.csv')
...
df_trends_monthly_aggregated.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/df_trends_monthly_YTD_YR.csv')
print(df_trends_monthly_aggregated.columns)
...
merged_df.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/merged_df.csv')
...
# Assume df_trends and other dataframes have been defined and contain the necessary data

# Function to create lagged features for all available lags
def create_all_lagged_features(df, group_col, value_cols):
    df_grouped = df.groupby(group_col)
    lags_df = []
    for value_col in value_cols:
        # Shift the column for each group to create lagged features
        for lag in range(1, len(df_grouped) + 1):
            lagged_col_name = f'{value_col}_lag{lag}'
            df[lagged_col_name] = df_grouped[value_col].shift(lag)
            # Optionally, fill missing values if needed
            # df[lagged_col_name].fillna(method='ffill', inplace=True)
        # Add to the list of lagged dataframes
        lags_df.append(df[[lagged_col_name for lag in range(1, len(df_grouped) + 1)]])
    return pd.concat(lags_df, axis=1)

# List of columns to create lagged features for
trends_value_cols = [col for col in df_trends.columns if col not in ['Country', 'Year', 'Month']]
rd_expenditure_value_cols = [col for col in rd_expenditure_df_rev.columns if col not in ['Country', 'Year', 'Month']]
oecd_value_cols = [col for col in oecd_df_rev_4lag.columns if col not in ['Country', 'Year']]

# Create lagged features for each group of columns
trends_lagged = create_all_lagged_features(merged_df, 'Country', trends_value_cols)
rd_expenditure_lagged = create_all_lagged_features(merged_df, 'Country', rd_expenditure_value_cols)
oecd_lagged = create_all_lagged_features(merged_df, 'Country', oecd_value_cols)

# Combine all lagged features into one dataframe
lagged_features_df = pd.concat([trends_lagged, rd_expenditure_lagged, oecd_lagged], axis=1)

# Combine the lagged features with the original dataframe
merged_df = pd.concat([merged_df, lagged_features_df], axis=1)

# Additional processing and filtering as necessary...
# For example, you might want to fill or drop NaNs, filter by year, etc.

# Save the final merged dataframe with all lagged features
merged_df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/merged_df_with_all_lags.csv")
...
merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/merged_df_test_RV01_batches_all_AggMGT_Evolution_0.csv")
#breakpoint()
...
X_all.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/X_all_AggMGT_Evolution.csv")
...
# Initialize an empty list to store the names of relevant columns
relevant_columns = []

# Add non-lagged columns that are always relevant
non_lagged_cols = ['Country', 'Year', 'Month']
# relevant_columns += [col for col in df_trends.columns if col not in non_lagged_cols]
# relevant_columns += [col for col in rd_expenditure_df_rev.columns if col not in non_lagged_cols]
# relevant_columns += [col for col in oecd_df_rev_4lag.columns if col not in non_lagged_cols]

# Append all lagged columns for trends
relevant_columns += [col for col in df_trends.columns if "_lag" in col]

# Append all lagged columns for RD expenditure
relevant_columns += [col for col in rd_expenditure_df_rev.columns if "_lag" in col]

# Append all lagged columns for OECD data
relevant_columns += [col for col in oecd_df_rev_4lag.columns if "_lag" in col]

# Now relevant_columns will have all columns that are not 'Country', 'Year', 'Month', and all lagged columns
...
X_train.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/X_train_std.csv")
...
class MLP(nn.Module): # MLP class from nn.Module/base class for all NN
    def __init__(self, input_dim, hidden1_dim, hidden2_dim, hidden3_dim, hidden4_dim, hidden5_dim, hidden6_dim, hidden7_dim, hidden8_dim, hidden9_dim, hidden10_dim, hidden11_dim, output_dim, num_countries, embedding_dim):
        ...
        self.fc4 = nn.Linear(hidden3_dim, hidden4_dim, bias=True)
        self.fc5 = nn.Linear(hidden4_dim, hidden5_dim, bias=True)
        self.fc6 = nn.Linear(hidden5_dim, hidden6_dim, bias=True)
        self.fc7 = nn.Linear(hidden6_dim, hidden7_dim, bias=True)
        self.fc8 = nn.Linear(hidden7_dim, hidden8_dim, bias=True)
        self.fc9 = nn.Linear(hidden8_dim, hidden9_dim, bias=True)
        self.fc10 = nn.Linear(hidden9_dim, hidden10_dim, bias=True)
        self.fc11 = nn.Linear(hidden10_dim, hidden11_dim, bias=True)        
        self.fc12 = nn.Linear(hidden11_dim, output_dim, bias=True)  # Output layer
        ...
        self.bn4 = nn.BatchNorm1d(hidden4_dim)
        self.bn5 = nn.BatchNorm1d(hidden5_dim)
        self.bn6 = nn.BatchNorm1d(hidden6_dim)
        self.bn7 = nn.BatchNorm1d(hidden7_dim)
        self.bn8 = nn.BatchNorm1d(hidden8_dim)
        self.bn9 = nn.BatchNorm1d(hidden9_dim)        
        self.bn10 = nn.BatchNorm1d(hidden10_dim)        
        self.bn11 = nn.BatchNorm1d(hidden11_dim)        
    ...
    def forward(self, x, country_indices):
        ...
        x = self.bn7(self.relu(self.fc7(x)))
        x = self.bn8(self.relu(self.fc8(x)))
        x = self.bn9(self.relu(self.fc9(x))) 
        x = self.bn10(self.relu(self.fc10(x))) 
        x = self.bn11(self.relu(self.fc11(x)))         
        x = self.fc12(x)  # No activation at the output layer for regression tasks
...
class Ensemble:
    def __init__(self, model_class, input_dim, hidden1_dim, hidden2_dim, hidden3_dim, hidden4_dim, hidden5_dim, hidden6_dim, hidden7_dim, hidden8_dim, hidden9_dim, hidden10_dim, hidden11_dim, output_dim, size_ensemble, num_countries, embedding_dim):
        ...
        for _ in range(size_ensemble):
            ...
            self.models.append(model_class(input_dim, hidden1_dim, hidden2_dim, hidden3_dim, hidden4_dim, hidden5_dim, hidden6_dim, hidden7_dim, hidden8_dim, hidden9_dim, hidden10_dim, hidden11_dim, output_dim, num_countries, embedding_dim))
    ...
    def train(self, X_train, y_train, X_val, y_val, country_indices_train, country_indices_val, criterion, optimizer_class, num_epochs=num_epochs, batch_size=batch_size, patience=patience):
        ...
        for idx, model in enumerate(self.models):
            ...
            best_model_path = f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/best_model_{idx}.pt"
...
# Create an ensemble
size_ensemble = 2
ensemble = Ensemble(MLP, input_dim, hidden1_dim, hidden2_dim, hidden3_dim, hidden4_dim, hidden5_dim, hidden6_dim, hidden7_dim, hidden8_dim, hidden9_dim, hidden10_dim, hidden11_dim, output_dim, size_ensemble=size_ensemble, num_countries=num_countries, embedding_dim=embedding_dim)
...
# Save to CSV
csv_path = "/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/ensemble_info.csv"
...
# Save to CSV
adjusted_r_squared_csv_path = "/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/adjusted_r_squared_values.csv"
...
csv_path = "/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/df_combined_pred_vs_true.csv"
...
for country in countries:
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/Predictions_vs_TrueValues_{country}.png")
...
def plot_scatter_per_country(df, title_prefix):
    ...
    for country in countries:
        ...
        plt.grid(True)
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/Predictions_vs_TrueValues_scatter_{title_prefix}_{country}.png")        
        plt.show()
...
# Add the R-squared values to your metrics DataFrame
metrics_df["Test_R2"] = test_r2_list + [ensemble_r2]

# Save back to CSV
metrics_df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/metrics_df_minibatchGD.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.train_losses):
    ...
    # Save to CSV
    df_train_loss.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/MLP_MinibatchGD_Training_Loss_Iterations_Model_{idx+1}.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.val_losses):
    ...
    # Save to CSV
    df_val_loss.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/MLP_MinibatchGD_Validation_Loss_Iterations_Model_{idx+1}.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.train_losses):
    ...
    plt.legend()
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/MLP_MinibatchGD_Training_Loss_Iterations_Model_{idx+1}.png")
    #plt.show()
...
for idx, model_losses in enumerate(ensemble.val_losses):
    ...
    plt.legend()
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/MLP_MinibatchGD_Validation_Loss_Iterations_Model_{idx+1}.png")
    #plt.show()
...
# Save to CSV
df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/country_metrics.csv", index=False)
...
plt.savefig("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/country_metrics_vis.png")
#plt.show()
...
csv_path_elasticities = "/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/elasticities_avg_ensemble.csv"
df_elasticities_avg.to_csv(csv_path_elasticities, index=False)

csv_path_elasticities_raw = "/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AllVar/test10_ext_r2_allinfo/elasticities_all_models.csv"
