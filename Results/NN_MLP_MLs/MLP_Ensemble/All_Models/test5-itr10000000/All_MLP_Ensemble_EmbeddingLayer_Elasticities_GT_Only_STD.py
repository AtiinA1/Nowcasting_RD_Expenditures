...
df_trends_monthly_aggregated.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/df_trends_monthly_YTD.csv')
...
df_trends_monthly_aggregated.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/df_trends_monthly_YTD_YR.csv')
print(df_trends_monthly_aggregated.columns)
...
merged_df.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/merged_df.csv')
...
merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/merged_df_test_RV01_batches_all_AggMGT_Evolution_0.csv")
breakpoint()
...
X_all.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/X_all_AggMGT_Evolution.csv")
...
#####################
#####################
# Preprocessing for XGBoost, RF, SVR, EN
...
countries_train = X_train['Country'].values
countries_val = X_val['Country'].values
countries_test = X_test['Country'].values
...
X_train.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/X_train_std.csv")
breakpoint()
######### Standardization of outpu #########
# y_train_org = y_train
# y_test_org = y_test
# y_val_org = y_val
...
#base_input_dim = X_train[:, :-1].shape[1]
...
# For the training set
# X_train_tensor = torch.FloatTensor(X_train[:, :-1])  # Exclude the last column
# country_indices_train = torch.LongTensor(X_train[:, -1])  # Take only the last column for country indices
# y_train_tensor = torch.FloatTensor(y_train)

# # For the validation set
# X_val_tensor = torch.FloatTensor(X_val[:, :-1])  # Exclude the last column
# country_indices_val = torch.LongTensor(X_val[:, -1])  # Take only the last column for country indices
# y_val_tensor = torch.FloatTensor(y_val)

# # For the test set
# X_test_tensor = torch.FloatTensor(X_test[:, :-1])  # Exclude the last column
# country_indices_test = torch.LongTensor(X_test[:, -1])  # Take only the last column for country indices
# y_test_tensor = torch.FloatTensor(y_test)
...
class Ensemble:
    ...
    def train(self, X_train, y_train, X_val, y_val, country_indices_train, country_indices_val, criterion, optimizer_class, num_epochs=num_epochs, batch_size=batch_size, patience=patience):
        ...
        for idx, model in enumerate(self.models):
            ...
            best_model_path = f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/best_model_{idx}.pt"
...
# Save to CSV
csv_path = "/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ensemble_info.csv"  # Modify this to your desired path
...
# Save back to CSV
metrics_df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/metrics_df_minibatchGD.csv", index=False)
...
# # Function to compute metrics on original scale
# def compute_metrics_on_original_scale(predictions, true_values, y_scaler):
#     predictions_original = y_scaler.inverse_transform(predictions.detach().numpy())
#     true_values_original = y_scaler.inverse_transform(true_values.detach().numpy())
...
#     mape_original = np.mean(np.abs((true_values_original - predictions_original) / true_values_original)) * 100
    
#     return rmse_original, mae_original, mape_original

# # Compute metrics for training data
# train_preds = ensemble.predict(X_train_tensor, country_indices_train)
# train_rmse, train_mae, train_mape = compute_metrics_on_original_scale(train_preds, y_train_tensor, y_scaler)

# # Compute metrics for validation data
# val_preds = ensemble.predict(X_val_tensor, country_indices_val)
# val_rmse, val_mae, val_mape = compute_metrics_on_original_scale(val_preds, y_val_tensor, y_scaler)

# # Re-assign the metrics
# train_rmse_list = [train_rmse] * size_ensemble  # since all models in ensemble will have the same training data metrics
# val_rmse_list = [val_rmse] * size_ensemble
# train_mae_list = [train_mae] * size_ensemble
# val_mae_list = [val_mae] * size_ensemble
# train_mape_list = [train_mape] * size_ensemble
# val_mape_list = [val_mape] * size_ensemble
# ensemble_val_rmse = val_rmse
# ensemble_val_mae = val_mae
# ensemble_val_mape = val_mape
...
#     original_predictions = y_scaler.inverse_transform(model_predictions.detach().numpy())
#     original_y_test = y_scaler.inverse_transform(y_test_tensor.detach().numpy())

#     model_rmse = ensemble.rmse(torch.tensor(original_predictions), torch.tensor(original_y_test))
#     model_mae = ensemble.mae(torch.tensor(original_predictions), torch.tensor(original_y_test))
#     model_mape = ensemble.mape(torch.tensor(original_predictions), torch.tensor(original_y_test))
...
# original_ensemble_predictions = y_scaler.inverse_transform(ensemble_predictions.detach().numpy())

# ensemble_test_rmse = ensemble.rmse(torch.tensor(original_ensemble_predictions), torch.tensor(original_y_test)).item()
# ensemble_test_mae = ensemble.mae(torch.tensor(original_ensemble_predictions), torch.tensor(original_y_test)).item()
# ensemble_test_mape = ensemble.mape(torch.tensor(original_ensemble_predictions), torch.tensor(original_y_test)).item()
...
# plt.savefig("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/Test_MGTs_MAPE_Month.png")
# #plt.show()
...
for idx, model_losses in enumerate(ensemble.train_losses):
    ...
    plt.legend()
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/MLP_MinibatchGD_Training_Loss_Iterations_Model_{idx+1}.png")
    #plt.show()
...
for idx, model_losses in enumerate(ensemble.val_losses):
    ...
    plt.legend()
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/MLP_MinibatchGD_Validation_Loss_Iterations_Model_{idx+1}.png")
    #plt.show()
...
# Save to CSV
df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/country_metrics.csv", index=False)
...
plt.savefig("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/country_metrics_vis.png")
#plt.show()
...
feature_names = list(X_train_standardized.drop(columns=['country_encoded']).columns)
...
decoded_country_names = le.inverse_transform(X_train_standardized['country_encoded'].astype(int).values)
# decoded_country_names = le.inverse_transform(X_train_standardized['country_encoded'].values)
...
csv_path_elasticities = "/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/elasticities_avg_ensemble.csv"
...
csv_path_elasticities_raw = "/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/elasticities_all_models.csv"
...
for lag in range(max_lag, max_lag+1):
    ...
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/feature_importance_plt_importance_{lag}.png", dpi=300)
    ...
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/feature_importance_norm_name_{lag}.png", dpi=300)
    plt.close()  # Close the current figure
    ...
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/model_residuals_train_{model_name}_{lag}.png")
    #plt.show()
    ...
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/model_scatter_test_{model_name}_{lag}.png")
    #plt.show()

    sns.set_theme(color_codes=True)
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.hist(y_test_lag['residuals'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/model_residuals_test_{model_name}_{lag}.png")
    #plt.show()
...
df_elasticities.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/elasticities_by_lag.csv', index=False)

merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/merged_df_test_RV01_batches_all_{model_name}.csv")

# Convert all_important_features to a DataFrame and save
df_all_important_features = pd.DataFrame(all_important_features)
df_all_important_features.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/all_important_features_by_lag_{model_name}.csv", index=False)
...
evaluation_train_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/evaluation_train_{model_name}.csv", index=False)
evaluation_test_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/evaluation_test_{model_name}.csv", index=False)
...
# sns.boxplot(x='Month', y='Percentage_Error', data=error_df)
# plt.title("Distribution of Percentage Errors by Month")
# plt.savefig("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/Test_MGTs_MAPE_Month.png")
# #plt.show()
...
country_evaluation_df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/country_evaluation.csv", index=False)
...
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.tight_layout()
plt.savefig("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/XGBoost/Eval_Month_vis.png")
#plt.show()
...
for lag in range(max_lag, max_lag+1):
    ...
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.scatter(y_train_lag['rd_expenditure'], y_train_lag['y_pred'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/model_scatter_train_{model_name}_{lag}.png")
    #plt.show()

    sns.set_theme(color_codes=True)
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.hist(y_train_lag['residuals'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/model_residuals_train_{model_name}_{lag}.png")
    #plt.show()
    ...
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.scatter(y_test_lag['rd_expenditure'], y_test_lag['y_pred'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/model_scatter_test_{model_name}_{lag}.png")
    #plt.show()

    sns.set_theme(color_codes=True)
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.hist(y_test_lag['residuals'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/model_residuals_test_{model_name}_{lag}.png")
    #plt.show()
...
#%%--------------------------------------------------------------------------
# Save the DataFrame to CSV
df_elasticities.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/elasticities_by_lag.csv', index=False)

merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/merged_df_test_RV01_batches_all_{model_name}.csv")

# Convert all_important_features to a DataFrame and save
df_all_important_features = pd.DataFrame(all_important_features)
df_all_important_features.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/all_important_features_by_lag_{model_name}.csv", index=False)
...
# Save the DataFrames to CSV files
evaluation_train_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/evaluation_train_{model_name}.csv", index=False)
evaluation_test_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/evaluation_test_{model_name}.csv", index=False)
...
# Save the results to CSV
country_evaluation_df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/country_evaluation.csv", index=False)
...
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.tight_layout()
plt.savefig("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/RF/Eval_Month_vis.png")
#plt.show()
...
for lag in range(max_lag, max_lag+1):
    ...
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.scatter(y_train_lag['rd_expenditure'], y_train_lag['y_pred'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/model_scatter_train_{model_name}_{lag}.png")
    #plt.show()

    sns.set_theme(color_codes=True)
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.hist(y_train_lag['residuals'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/model_residuals_train_{model_name}_{lag}.png")
    #plt.show()
    ...
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.scatter(y_test_lag['rd_expenditure'], y_test_lag['y_pred'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/model_scatter_test_{model_name}_{lag}.png")
    #plt.show()

    sns.set_theme(color_codes=True)
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.hist(y_test_lag['residuals'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/model_residuals_test_{model_name}_{lag}.png")
    #plt.show()
...
#%%--------------------------------------------------------------------------
# Save the DataFrame to CSV
df_elasticities.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/elasticities_by_lag.csv', index=False)

merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/merged_df_test_RV01_batches_all_{model_name}.csv")

# Convert all_important_features to a DataFrame and save
df_all_important_features = pd.DataFrame(all_important_features)
df_all_important_features.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/all_important_features_by_lag_{model_name}.csv", index=False)
...
# Save the DataFrames to CSV files
evaluation_train_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/evaluation_train_{model_name}.csv", index=False)
evaluation_test_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/evaluation_test_{model_name}.csv", index=False)
...
# Save the results to CSV
country_evaluation_df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/country_evaluation.csv", index=False)
...
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.tight_layout()
plt.savefig("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/SVR/Eval_Month_vis.png")
#plt.show()
...
for lag in range(max_lag, max_lag+1):
    ...
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.scatter(y_train_lag['rd_expenditure'], y_train_lag['y_pred'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/model_scatter_train_{model_name}_{lag}.png")
    #plt.show()

    sns.set_theme(color_codes=True)
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.hist(y_train_lag['residuals'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/model_residuals_train_{model_name}_{lag}.png")
    #plt.show()
    ...
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.scatter(y_test_lag['rd_expenditure'], y_test_lag['y_pred'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/model_scatter_test_{model_name}_{lag}.png")
    #plt.show()

    sns.set_theme(color_codes=True)
    fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
    plt.hist(y_test_lag['residuals'])
    plt.savefig(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/model_residuals_test_{model_name}_{lag}.png")
    #plt.show()
...
#%%--------------------------------------------------------------------------
# Save the DataFrame to CSV
df_elasticities.to_csv('/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/elasticities_by_lag.csv', index=False)

merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/merged_df_test_RV01_batches_all_{model_name}.csv")

# Convert all_important_features to a DataFrame and save
df_all_important_features = pd.DataFrame(all_important_features)
df_all_important_features.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/all_important_features_by_lag_{model_name}.csv", index=False)
...
# Save the DataFrames to CSV files
evaluation_train_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/evaluation_train_{model_name}.csv", index=False)
evaluation_test_df.to_csv(f"/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/evaluation_test_{model_name}.csv", index=False)
...
# Save the results to CSV
country_evaluation_df.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/country_evaluation.csv", index=False)
...
plt.grid(True, which='both', linestyle='--', linewidth=0.5)
plt.tight_layout()
plt.savefig("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/ElasticNet/Eval_Month_vis.png")
...
df_metrics.to_csv("/Users/atin/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/std/meta_model_metrics.csv", index=False)
