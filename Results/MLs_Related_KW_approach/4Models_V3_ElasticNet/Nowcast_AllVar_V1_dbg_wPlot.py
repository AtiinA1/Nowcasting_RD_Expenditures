...
df_trends= pd.read_csv('./data/GT/related_V02/final_df_BusinessRnDEcosystem_merged_resampled.csv')
df_trends_monthly = pd.read_csv('./data/GT/related_V02/final_df_BusinessRnDEcosystem_merged.csv')
...
df_trends_monthly_aggregated.to_csv('/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/df_trends_monthly_YTD.csv')
...
df_trends_monthly_aggregated.to_csv('/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/df_trends_monthly_YTD_YR.csv')
print(df_trends_monthly_aggregated.columns)
...
merged_df.to_csv('/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/merged_df.csv')
...
merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/merged_df_test_RV01_batches_all_{model_name}_0.csv")
...
for country in X_all['Country'].unique():
    ...
    for lag in range(1, max_lag+1):
        ...
        # Keep only the relevant lagged columns
        ...
        #relevant_columns += ['Country_label', 'Year']
        ...
        #model_coeff.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/model_coeff_{country}_{lag}.csv")
        ...
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/model_scatter_train_{model_name}_{country}_{lag}.png")
        #plt.show()

        sns.set_theme(color_codes=True)
        fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
        plt.hist(y_train_country['residuals'])
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/model_residuals_train_{model_name}_{country}_{lag}.png")
        #plt.show()
        ...
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/model_scatter_test_{model_name}_{country}_{lag}.png")
        #plt.show()
    
        sns.set_theme(color_codes=True)
        fig, ax = plt.subplots(figsize=(10, 8), nrows=1, ncols=1)
        plt.hist(y_test_country['residuals'])
        plt.savefig(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/model_residuals_test_{model_name}_{country}_{lag}.png")
        #plt.show()
...
merged_df.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/merged_df_test_RV01_batches_all_{model_name}.csv")

# Create DataFrames to store the MAE and RMSE results for each country and each lag
evaluation_train_df = pd.DataFrame(columns=['Country', 'Lag', 'MAE', 'RMSE'])
evaluation_test_df = pd.DataFrame(columns=['Country', 'Lag', 'MAE', 'RMSE'])
...
for country in rmse_dict.keys():
    for lag in rmse_dict[country].keys():
        train_row = {'Country': country, 'Lag': lag, 'MAE': mae_dict[country][lag], 'RMSE': rmse_dict[country][lag]}
        evaluation_train_df = evaluation_train_df.append(train_row, ignore_index=True)
        test_row = {'Country': country, 'Lag': lag, 'MAE': mae_dict_test[country][lag], 'RMSE': rmse_dict_test[country][lag]}
        evaluation_test_df = evaluation_test_df.append(test_row, ignore_index=True)

# Save the DataFrames to CSV files
evaluation_train_df.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/evaluation_batches_train_{model_name}.csv", index=False)
evaluation_test_df.to_csv(f"/Users/atin/Nowcasting/data/Results/4Models_V3/AllVar/evaluation_testAGT1_{model_name}.csv", index=False)
