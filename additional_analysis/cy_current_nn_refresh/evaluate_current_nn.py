...
feat = pd.read_csv(os.path.join(SOURCE_OUT, "merged_features.csv"))
...
for cfg in CONFIGS:
    ...
    annual=d.groupby(["Country","Year"],as_index=False).agg(
        {"True_Values":"mean", **{c:"mean" for c in member_cols}}
    )
    M=annual[member_cols].values
    lo=np.quantile(M,0.025,axis=1); hi=np.quantile(M,0.975,axis=1)
    mu=M.mean(1); sd=M.std(1)
    actual=annual.True_Values.values
    cov_q=np.mean((actual>=lo)&(actual<=hi))*100
    cov_g=np.mean((actual>=mu-1.96*sd)&(actual<=mu+1.96*sd))*100
