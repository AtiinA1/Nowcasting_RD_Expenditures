"""PREVIEW: NNvsOLS_Temporal with per-configuration colors; NN=solid, OLS=hatched."""
...
CONFIG_COLORS={"LagRD":"#969696","Macros":"#e08214","AGT":"#ffd92f","MGT":"#66c2a5",
               "AGTwRD":"#fc8d62","MGTwRD":"#b3b3e6","AllVar":"#3a7fc0"}
...
for k,(ni,oi,lab) in enumerate([(0,1,"MAPE (%)"),(2,3,"RMSE (USD bn)")]):
    ...
    for lbl in ax[k].get_xticklabels(): lbl.set_color(CONFIG_COLORS[lbl.get_text()]); lbl.set_fontweight("bold")
leg=[Patch(facecolor="#888",edgecolor="#333",label="Neural network (solid)"),
     Patch(facecolor="#888",edgecolor="#333",alpha=0.5,hatch="////",label="OLS, same inputs (hatched)")]
...
plt.tight_layout(); plt.savefig(os.path.join(FT,"_preview_NNvsOLS_configcolors.png")); print("saved preview")
