...
df_trends= pd.read_csv('/Nowcasting/data/GT/related_toic_approach/trends_data_by_topic_resampled_filtered.csv')
df_trends_monthly = pd.read_csv('/Nowcasting/data/GT/related_toic_approach/trends_data_by_topic_filtered.csv')
...
df_trends_monthly_aggregated.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/df_trends_monthly_YTD.csv')
...
df_trends_monthly_aggregated.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/df_trends_monthly_YTD_YR.csv')
print(df_trends_monthly_aggregated.columns)
...
merged_df.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/merged_df.csv')
...
merged_df.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/merged_df_test_RV01_batches_all_AggMGT_Evolution_0.csv")
...
X_all.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/X_all_AggMGT_Evolution.csv")
...
def zero_out_non_relevant_features(df, relevant_features):
    for col in df.columns:
        if col not in relevant_features:
            df[col] = 0  # Set non-relevant feature values to 0
    return df

# Assuming `current_relevant_features` is a list of relevant features for the current configuration
X_train_config = zero_out_non_relevant_features(X_train.copy(), relevant_columns)
X_val_config = zero_out_non_relevant_features(X_val.copy(), relevant_columns)
X_test_config = zero_out_non_relevant_features(X_test.copy(), relevant_columns)
...
X_train.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/X_train_OHC_Month_Cat_Country.csv")
...
X_train.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/X_train_std.csv")
...
class Ensemble:
    ...
    def train(self, X_train, y_train, X_val, y_val, country_indices_train, country_indices_val, criterion, optimizer_class, num_epochs=num_epochs, batch_size=batch_size, patience=patience):
        ...
        self.grad_norms = [[] for _ in range(size_ensemble)]
        ...
        for idx, model in enumerate(self.models):
            ...
            best_model_path = f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/best_model_{idx}.pt"
            ...
            for epoch in range(num_epochs):
                ...
                for i in range(0, len(X_train), batch_size):
                    ...
                    # Step 2: Calculate the norm of gradients for all model parameters
                    total_norm = 0
                    for param in model.parameters():
                        if param.grad is not None:
                            param_norm = param.grad.data.norm(2)  # L2 norm
                            total_norm += param_norm.item() ** 2
                    total_norm = total_norm ** 0.5  # Take square root to get the overall norm
                    
                    # Step 3: Store the calculated norm
                    self.grad_norms[idx].append(total_norm)
...
# Save to CSV
csv_path = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/ensemble_info.csv"
...
adjusted_r_squared_csv_path = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/adjusted_r_squared_values.csv"
...
csv_path = "/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/df_combined_pred_vs_true.csv"
...
def plot_scatter_per_country(df, title_prefix):
    ...
    for country in countries:
        ...
        plt.grid(True)
        plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/{title_prefix}_Predictions_vs_TrueValues_{country}.pdf")        
    ...
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/{title_prefix}_Predictions_vs_TrueValues_Combined.pdf")
    plt.close()  # Close the individual plot
...
def plot_scatter_per_country_log(df, title_prefix):
    ...
    for country in countries:
        ...
        plt.grid(True)
        plt.yscale('log')
        plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/{title_prefix}_Predictions_vs_TrueValues_{country}_log.pdf")        
        plt.close()  # Close the individual plot
    ...
    plt.yscale('log')
    plt.legend(title="Country")
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/{title_prefix}_Predictions_vs_TrueValues_Combined_log.pdf")
    plt.close()  # Close the individual plot
...
# Save back to CSV
metrics_df.to_csv("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/metrics_df_minibatchGD.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.train_losses):
    ...
    # Save to CSV
    df_train_loss.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/MLP_MinibatchGD_Training_Loss_Iterations_Model_{idx+1}.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.val_losses):
    ...
    # Save to CSV
    df_val_loss.to_csv(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/MLP_MinibatchGD_Validation_Loss_Iterations_Model_{idx+1}.csv", index=False)
...
for idx, model_losses in enumerate(ensemble.train_losses):
    ...
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/MLP_MinibatchGD_Training_Loss_Iterations_Model_{idx+1}.pdf")
    #plt.show()
...
for idx, model_losses in enumerate(ensemble.val_losses):
    ...
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/MLP_MinibatchGD_Validation_Loss_Iterations_Model_{idx+1}.pdf")
    #plt.show()
...
# Save to CSV
df.to_csv("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/country_metrics.csv", index=False)
...
for model in df['Models'][0]:
    ...
    plt.savefig(f"/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/country_metrics_vis_{model}.pdf")
    #plt.show()
...
# Load the data
csv_path_elasticities_avg_woyr = '/Nowcasting/data/Results/AggMGT_Evolution/MLP_Ensemble_10_Final/XGBoost_MLP_Meta_Model/AGT/test10_related_topic_diff_states_10ensemble/elasticities_avg_ensemble_woyr.csv'
df_elasticities_avg_woyr = pd.read_csv(csv_path_elasticities_avg_woyr)

# Save to CSV if needed
df_elasticities = df_elasticities_avg_woyr
...
# Extract entries with '_yearly_avg_lag' from df_elasticities
df_elasticities_lagged = df_elasticities[df_elasticities['Feature'].str.contains('_yearly_avg_lag')]

# Extract the base column name from the "Feature" column by removing the '_yearly_avg_lag' suffix
df_elasticities_lagged['base_feature'] = df_elasticities_lagged['Feature'].str.replace('_yearly_avg_lag[123]', '')

# Average elasticities for each feature across _yearly_avg_lag1, _yearly_avg_lag2, _yearly_avg_lag3
df_avg_elasticities = df_elasticities_lagged.groupby(['Country', 'base_feature']).agg({
    'Elasticity': 'mean',
    'Elasticity_Lower_Bound': 'mean',
    'Elasticity_Upper_Bound': 'mean'
}).reset_index()

# Rename the features to have _yearly_avg_lag1
df_avg_elasticities['Feature'] = df_avg_elasticities['base_feature'] + '_yearly_avg_lag1'
df_avg_elasticities = df_avg_elasticities.drop(columns=['base_feature'])

df_avg_elasticities = df_avg_elasticities.drop_duplicates(subset=['Country', 'Feature'])

# Save the averaged elasticities DataFrame to a CSV file
df_avg_elasticities.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/averaged_elasticities_over_lags.csv', index=False)

df_avg_elasticities = pd.read_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/averaged_elasticities_over_lags.csv')
...
df_elasticities = df_avg_elasticities

# Filter df_elasticities to include only entries with '_yearly_avg_lag'
df_elasticities_lagged = df_elasticities[df_elasticities['Feature'].str.contains('_yearly_avg_lag')]

# Extract the base column name from the "Feature" column by removing the '_yearly_avg_lag' suffix
df_elasticities_lagged['base_feature'] = df_elasticities_lagged['Feature'].str.replace('_yearly_avg', '')

df_elasticities_lagged.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/df_elasticities_lagged.csv', index=False)
df_trends_monthly.to_csv('/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/df_trends_monthly.csv', index=False)

# Define lags
lags = [1]

# Get the unique list of countries from the DataFrame
countries = df_trends_monthly['Country'].unique()

for country in countries:
    df_country = df_trends_monthly[df_trends_monthly['Country'] == country]
    
    for col in google_trend_cols:
        for lag in lags:
            lagged_col = f"{col}_lag{lag}"
            
            # Find the corresponding elasticity values for this country and feature
            elasticity_value = df_elasticities_lagged[(df_elasticities_lagged['base_feature'] == lagged_col) & (df_elasticities_lagged['Country'] == country)]['Elasticity'].values[0]
            elasticity_lower_value = df_elasticities_lagged[(df_elasticities_lagged['base_feature'] == lagged_col) & (df_elasticities_lagged['Country'] == country)]['Elasticity_Lower_Bound'].values[0]
            elasticity_upper_value = df_elasticities_lagged[(df_elasticities_lagged['base_feature'] == lagged_col) & (df_elasticities_lagged['Country'] == country)]['Elasticity_Upper_Bound'].values[0]
            
            # Assign the elasticity values to all rows for this country and feature
            df_trends_monthly.loc[df_trends_monthly['Country'] == country, f"{lagged_col}_elasticity"] = elasticity_value
            df_trends_monthly.loc[df_trends_monthly['Country'] == country, f"{lagged_col}_elasticity_lower"] = elasticity_lower_value
            df_trends_monthly.loc[df_trends_monthly['Country'] == country, f"{lagged_col}_elasticity_upper"] = elasticity_upper_value
            df_trends_monthly.loc[df_trends_monthly['Country'] == country, f'{col}_adjusted_proportion'] = elasticity_value * df_country[f'{col}_proportion']
            df_trends_monthly.loc[df_trends_monthly['Country'] == country, f'{col}_adjusted_proportion_lower'] = elasticity_lower_value * df_country[f'{col}_proportion']
            df_trends_monthly.loc[df_trends_monthly['Country'] == country, f'{col}_adjusted_proportion_upper'] = elasticity_upper_value * df_country[f'{col}_proportion']
...
df_trends_monthly['total_adjusted_proportion_lower'] = df_trends_monthly[[f'{col}_adjusted_proportion_lower' for col in google_trend_cols]].sum(axis=1)
df_trends_monthly['total_adjusted_proportion_upper'] = df_trends_monthly[[f'{col}_adjusted_proportion_upper' for col in google_trend_cols]].sum(axis=1)
...
df_trends_monthly['total_adjusted_proportion_yearly_lower'] = df_trends_monthly.groupby(['Year', 'Country'])['total_adjusted_proportion_lower'].transform('sum')
df_trends_monthly['total_adjusted_proportion_yearly_upper'] = df_trends_monthly.groupby(['Year', 'Country'])['total_adjusted_proportion_upper'].transform('sum')

df_trends_monthly = pd.merge(df_trends_monthly, rd_expenditure_df_rev, on=['Country', 'Year'], how='left')
...
df_trends_monthly['monthly_rd_expenditure_lower'] = df_trends_monthly['rd_expenditure'] * (df_trends_monthly['total_adjusted_proportion_lower'] / df_trends_monthly['total_adjusted_proportion_yearly_lower'])
df_trends_monthly['monthly_rd_expenditure_upper'] = df_trends_monthly['rd_expenditure'] * (df_trends_monthly['total_adjusted_proportion_upper'] / df_trends_monthly['total_adjusted_proportion_yearly_upper'])

# Save to CSV
df_trends_monthly.to_csv("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/df_trends_monthly_elast_adjusted_with_bounds.csv", index=False)

#####################################

df_monthly_predictions = pd.merge(df_trends_monthly, df_combined, on=['Year', 'Month', 'Country'], how='left')

df_monthly_predictions['monthly_rd_expenditure_onpreds'] = df_monthly_predictions['Predicted_Values'] * (df_monthly_predictions['total_adjusted_proportion'] / df_monthly_predictions['total_adjusted_proportion_yearly'])

df_monthly_predictions.to_csv("/Nowcasting/nn_mlp_nowcasting_model/results/AGT/4TD/df_trends_monthly_elast_adjusted_onpreds_with_bounds.csv", index=False)
